...
function RequestModal({ isOpen, onClose, userId, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.agreement) { setError('Необходимо согласие на обработку данных'); return; }
  ...
  return (
    <div className="auth-overlay">
      <div className="auth-modal" style={{ maxWidth: '600px', width: '90%' }}>
        ...
        <h3 style={{ marginBottom: '1rem', color: 'var(--blue)' }}>Новая заявка на оценку</h3>
        <form onSubmit={handleSubmit}>
          <input 
            ...
            disabled={isLoading} 
            style={{ width: '100%', marginBottom: '1rem', padding: '12px 16px', borderRadius: '30px', border: '1px solid var(--border)', fontFamily: 'Inter, sans-serif', fontSize: '0.9rem' }}
          />
          ...
          <select 
            ...
            onChange={handleChange} 
            disabled={isLoading} 
            style={{ width: '100%', marginBottom: '1rem', padding: '12px 16px', borderRadius: '30px', border: '1px solid var(--border)', fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', background: 'white' }}
          >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
            ...
            />
            <label htmlFor="has_restrictions" style={{ fontSize: '0.85rem', color: 'var(--text-muted)', cursor: 'pointer', margin: 0 }}>
          ...
          <textarea 
            ...
            onChange={handleChange} 
            disabled={isLoading} 
            style={{ width: '100%', marginBottom: '1rem', padding: '12px 16px', borderRadius: '20px', border: '1px solid var(--border)', fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', resize: 'vertical' }}
          ></textarea>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
            <input 
              ...
              style={{ width: '18px', height: '18px', cursor: 'pointer', margin: 0 }}
            />
            <label htmlFor="modalAgreement" style={{ fontSize: '0.8rem', color: 'var(--text-muted)', cursor: 'pointer', margin: 0 }}>
              Я соглашаюсь на <a href="#" style={{ color: 'var(--blue)' }}>обработку персональных данных</a>
            </label>
          </div>
          
          {error && <div style={{ color: '#c44', fontSize: '0.8rem', marginBottom: '1rem' }}>{error}</div>}
